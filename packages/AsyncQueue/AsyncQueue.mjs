 ...
 * Sample of asynchronous iteration from
 * the blog of Dr. Axel Rauschmayer, https://2ality.com
 *
 *   * An asynchronously iterable queue class.
 *   Add values with enqueue() and remove them with dequeue().
 *
 *   * dequeue() returns a Promise, which means that values
 *   can be dequeued before they are enqueued.
 *
 *   * The class implements [Symbol.asyncIterator] and next()
 *   so that it can be used with the for/await loop
 *   (which will not terminate until the close() method is called.)
****************************************************/

class AsyncQueue {

    // A sentinel value returned by dequeue() to mark "end of stream" when closed
    static EOQ = Symbol('end-of-queue');
    ...
    dequeue () {
        if( this.values.length > 0 ) {
            ...
            return Promise.resolve( AsyncQueue.EOQ );
        }
        else {
    ...
    close () {
        ...
        while( this.resolvers.length > 0 ) {
            this.resolvers.shift()( AsyncQueue.EOQ );
        }
        this.closed = true;
    }
    ...
    [Symbol.asyncIterator] () {
        //dtmp('invoke AsyncQueue[ Symbol.asyncIterator ]');
        return this;
    }
